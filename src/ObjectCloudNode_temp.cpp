...
ObjectCloudNode::ObjectCloudNode()
{
	...
	objectMapData_pub = nh.advertise<rtabmap_ros::MapData>("/object_MapData",1);
	//cout<<"init"<<endl;
 ...
 */
void ObjectCloudNode::mapDataCb(rtabmap_ros::MapDataPtr mapDataPtr)
{
	//currKfIDs.clear();
...
/*
	...
	for(int i=0;i<kfsToRemove.size();i++)
		kfObjectImages.erase(kfsToRemove[i]);*/

	//objectCloud.points.clear();
	for(int i=0;i<mapDataPtr->nodes.size();i++)
	{
		it = kfObjectImages.find(mapDataPtr->nodes[i].id);
		if(it != kfObjectImages.end())
			//objectCloud.points.insert(objectCloud.points.end(), it->second.points.begin(), it->second.points.end());
		{
			mapDataPtr->nodes[i].image = it->second.first;
			mapDataPtr->nodes[i].depth = it->second.second;
		}
		else
		{
			...
			pair<vector<unsigned char>, vector<unsigned char> > result = detectedImage(obj.response.objects, *(depthPtr->toImageMsg()), *(rgbPtr->toImageMsg()));
			kfObjectImages[mapDataPtr->nodes[i].id] = result;
			mapDataPtr->nodes[i].image = result.first;
			mapDataPtr->nodes[i].depth = result.second;
	...
	objectMapData_pub.publish(mapDataPtr);
...
pair<vector<unsigned char>, vector<unsigned char> > ObjectCloudNode::detectedImage(darknet_ros::DetectedObjects detections, sensor_msgs::Image depth, sensor_msgs::Image rgb)	
{
	try 
	{
		...
		//return cloud;
		return make_pair(rtabmap::compressImage(rgbPtr_->image), rtabmap::compressImage(depthPtr_->image));
